...
public final class SessionUtil {
	...
	*/
	public static void setValidateCode(HttpServletRequest req, String validateCode){
		setAttr(req, SESSION_VALIDATECODE, validateCode);
	}
	...
	*/
	private static String getValidateCode(){
		return (String)getAttr(SESSION_VALIDATECODE);
	}
	...
	* @return SysUser
	*/
	private static void removeValidateCode(){
	...
	*/
    ...
    /**
     * 判断验证码是否正确,验证码只可以使用一次 
     */
	public static  boolean checkcode(String verifyCode){
		String vcode  = getValidateCode();
		boolean flag=false;
		if(vcode==null||vcode.equals("")){
		}
		else
		{
			vcode=vcode.toLowerCase();  // session中的验证码
			removeValidateCode(); // 清除验证码，确保验证码只能用一次
		}
		if(verifyCode.toLowerCase().equals(vcode)){//判断验证码是否正确
			flag=true;
		}
		return flag;
	}
